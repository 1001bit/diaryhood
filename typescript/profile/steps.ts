...
// Count steps by paths stats
for (let i = 0; i < pathElems.length; i++) {
	const pathElem = pathElems[i] as HTMLDivElement;
	const statsData = JSON.parse(pathElem.getAttribute("stats-data") as string);
	...
	const pathStepsElem = pathElem.querySelector(
		".path-steps"
	) as HTMLParagraphElement;
